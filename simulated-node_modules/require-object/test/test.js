...
describe('folder structure dependent', () => {
	...
	describe('directory-structure-to-object.js', () => {
		...
		describe('aliasing', () => {
			...
			it('aliases conflicting punctuation with alphabetical priority', () => {

				assert.equal(dirObj['test-files'].conflicting.fileFile.toString(), 'file-file\n');

			});

			it('aliases are non-enumerable', () => {

				Object.keys(dirObj['test-files']).forEach(name => {
					assert.equal(['conflicting', 'js-file.js', 'json-file.json', 'text-file.txt'].includes(name), true);
				});
